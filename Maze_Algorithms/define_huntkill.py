...
class define_Huntkill:
	def check_position(self, checkedPosition, prePositions):
		for prePosition in prePositions:
			if checkedPosition == prePosition:
				return True
		if checkedPosition[0] == self.mazeSize or checkedPosition[0] < 0:
			return True
		if checkedPosition[1] == self.mazeSize or checkedPosition[1] < 0:
			return True
		return False
			...
			for row in self.maze:
				for column in row:
					if column[0] and column[1] and column[2] and column[3]:
						return True
			return False
		if position[1] + 1 < self.mazeSize:
			box = self.maze[position[1] + 1][position[0]]
			if box[0] and box[1] and box[2] and box[3]:
				return True
		if position[0] + 1 < self.mazeSize:
			box = self.maze[position[1]][position[0] + 1]
			if box[0] and box[1] and box[2] and box[3]:
				return True
		return False
	...
	def create_maze(self):
		while self.hunting:
			...
			if closedBox:
				...
				break
			if self.position[0] + 1 < self.mazeSize:
				self.position[0] += 1
			else:
		...
		if closedBox:
			...
			if self.position in self.oldPositions:
				canAppend = False
			if canAppend == True:
				self.oldPositions.append(self.position.copy())
			while canMove == False:
				...
				if cantMove[0] and cantMove[1] and cantMove[2] and cantMove[3]:
					self.position = self.huntPosition.copy()
					self.hunting = True
					break
